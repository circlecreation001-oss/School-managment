...
export class HomeworkRepository {
  async list(tenantId: string, params: { page: number; limit: number; classId?: string; subjectId?: string; status?: string; teacherId?: string }) {
    ...
    const [data, total] = await Promise.all([
      prisma.homework.findMany({ where, include: { class: { select: { name: true } }, subject: { select: { name: true } }, teacher: { select: { firstName: true, lastName: true } }, _count: { select: { submissions: true } } }, skip: (params.page - 1) * params.limit, take: params.limit, orderBy: { dueDate: 'desc' } }),
  ...
  async create(data: Prisma.HomeworkUncheckedCreateInput) { return prisma.homework.create({ data }); }
  async update(id: string, data: Prisma.HomeworkUpdateInput) { return prisma.homework.update({ where: { id }, data }); }
  async delete(id: string) { return prisma.homework.update({ where: { id }, data: { deletedAt: new Date(), status: 'archived' } }); }

  async addAttachment(data: Prisma.HomeworkAttachmentUncheckedCreateInput) { return prisma.homeworkAttachment.create({ data }); }
  ...
  async createSubmission(data: Prisma.SubmissionUncheckedCreateInput) { return prisma.submission.create({ data }); }
  async updateSubmission(id: string, data: Prisma.SubmissionUpdateInput) { return prisma.submission.update({ where: { id }, data }); }
