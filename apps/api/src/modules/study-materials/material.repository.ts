...
export class MaterialRepository {
  async list(tenantId: string, params: { page?: number; limit?: number; search?: string; classId?: string; subjectId?: string; batchId?: string; category?: string }) {
    ...
    const [data, total] = await Promise.all([
      prisma.studyMaterial.findMany({ where, include: { class: { select: { name: true } }, subject: { select: { name: true } }, teacher: { select: { firstName: true, lastName: true } } }, skip: ((params.page || 1) - 1) * (params.limit || 20), take: params.limit || 20, orderBy: { createdAt: 'desc' } }),
  ...
  async create(data: any /* Prisma.StudyMaterialUncheckedCreateInput */) { return prisma.studyMaterial.create({ data }); }
  async update(id: string, data: any /* Prisma.StudyMaterialUpdateInput */) { return prisma.studyMaterial.update({ where: { id }, data }); }
