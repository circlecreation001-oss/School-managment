...
export default function StudentExamsPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      setLoading(true);
      const res = await apiClient.get<ExamItem[]>(`/exams?classId=${classId}&limit=20`);
      if (res.success) {
              ...
              {exams.map((ex) => (
                <tr key={ex.id}>
                  ...
                  <td className="px-4 py-3">
                    <span className={`inline-flex rounded-full px-2 py-0.5 text-xs font-medium capitalize ${statusColor[ex.status] || 'bg-slate-100'}`}>
                      {ex.status?.replace('_', ' ')}
                    </span>
                  </td>
                </tr>
