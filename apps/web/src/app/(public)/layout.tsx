...
export default function PublicLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen flex flex-col">
        ...
        <nav className="mx-auto max-w-7xl flex items-center justify-between px-4 sm:px-6 lg:px-8 h-16 lg:h-[72px]">
          ...
          <div className="hidden lg:flex items-center gap-1">
            ...
            <div className="relative" onMouseEnter={() => setMegaOpen('solutions')} onMouseLeave={() => setMegaOpen(null)}>
              <button className={`px-3 py-2 rounded-lg text-sm font-medium transition-all flex items-center gap-1 ${scrolled ? 'text-slate-600 hover:text-blue-600 hover:bg-blue-50' : 'text-white/80 hover:text-white'}`}>
                Solutions
          ...
          <div className="hidden lg:flex items-center gap-3">
            <Link href="/login" className={`text-sm font-medium px-4 py-2 rounded-lg transition-all ${scrolled ? 'text-slate-700 hover:text-blue-600 hover:bg-blue-50' : 'text-white/90 hover:text-white hover:bg-white/10'}`}>
              Portal Login</Link>
            <Link href="/signup" className={`text-sm font-medium px-4 py-2 rounded-lg transition-all border ${scrolled ? 'text-blue-600 border-blue-200 hover:bg-blue-50' : 'text-white border-white/30 hover:bg-white/10'}`}>
      ...
      {/* WhatsApp Chat */}
      <a href="https://wa.me/919572495969?text=Hello%20SchoolNex%20Team,%20I'm%20interested%20in%20your%20School%20Management%20Software.%20Please%20share%20more%20details." target="_blank" rel="noopener noreferrer"
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-8 pb-12 border-b border-slate-800/60">
            <div className="col-span-2 lg:col-span-1">
              <div className="flex items-center gap-2.5 mb-4">
                <div className="h-8 w-8 rounded-xl bg-gradient-to-br from-blue-500 to-indigo-500 flex items-center justify-center"><span className="text-white font-bold text-xs">S</span></div>
                <span className="text-lg font-bold">SchoolNex</span>
              </div>
              <p className="text-sm text-slate-400 leading-relaxed mb-3">Complete Education Management Platform for schools, colleges, and coaching institutes.</p>
              <p className="text-xs text-slate-500">Developed by Circle Creation</p>
              <p className="text-xs text-slate-500">Founder: Shivam Kumar</p>
            </div>
            <div>
            ...
            <div>
              ...
              <ul className="space-y-2.5 text-sm text-slate-400">
                ...
                <li><Link href="/contact" className="hover:text-white transition-colors">Book Demo</Link></li>
              </ul>
            </div>
            <div>
          ...
          <div className="mt-8 flex flex-col md:flex-row items-center justify-between gap-4">
            <div className="text-center md:text-left">
              <p className="text-xs text-slate-500">SchoolNex &middot; Developed by Circle Creation &middot; Founder: Shivam Kumar</p>
              <p className="text-xs text-slate-600 mt-1">&copy; {new Date().getFullYear()} All Rights Reserved.</p>
            </div>
